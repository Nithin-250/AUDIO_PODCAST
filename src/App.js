...
// Simple translation helper using LibreTranslate-compatible endpoints (no API key).
// Adds provider fallback and chunking for long texts. Falls back to original text on error.
async function translateText(text, targetLang) {
  ...
  // Split text into manageable chunks to avoid provider limits
  const splitIntoChunks = (t, max = 4000) => {
    const parts = [];
    let current = '';
    const segments = (t.match(/[^.!?\n\r\t]+[.!?\n\r\t]*/g) || [t]);
    for (const seg of segments) {
      if ((current + seg).length > max && current) {
        parts.push(current);
        current = seg;
      } else {
        current += seg;
  ...
  for (const url of providers) {
    try {
      ...
      for (const chunk of chunks) {
        ...
        const piece = data?.translatedText || data?.translated_text;
        if (!piece) throw new Error('no translation content');
        out.push(piece);
      }
      return out.join(' ');
    } catch (_) {
